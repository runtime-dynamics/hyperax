...
// PipelineRepo implements repo.PipelineRepo for MySQL.
...
func (r *PipelineRepo) CreatePipeline(ctx context.Context, pipeline *repo.Pipeline) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.PipelineRepo.CreatePipeline: %w", err)
	}
...
func (r *PipelineRepo) GetPipeline(ctx context.Context, id string) (*repo.Pipeline, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.GetPipeline: %w", err)
	}
...
func (r *PipelineRepo) ListPipelines(ctx context.Context, workspaceName string) ([]*repo.Pipeline, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.ListPipelines: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyPipelines(rows)
}
...
func (r *PipelineRepo) CreateJob(ctx context.Context, pipelineID, workspaceName string) (string, error) {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO pipeline_jobs (id, pipeline_id, status, workspace_name, started_at)
		 VALUES (?, ?, 'pending', ?, NOW())`, id, pipelineID, workspaceName,
	)
	if err != nil {
		return "", fmt.Errorf("mysql.PipelineRepo.CreateJob: %w", err)
	}
	return id, nil
}
...
func (r *PipelineRepo) GetJob(ctx context.Context, id string) (*repo.PipelineJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.GetJob: %w", err)
	}
	if startedAt.Valid {
...
func (r *PipelineRepo) UpdateJobStatus(ctx context.Context, id string, status string, result string) error {
	...
	if status == "completed" || status == "failed" || status == "cancelled" {
		res, err = r.db.ExecContext(ctx,
			`UPDATE pipeline_jobs SET status = ?, result = ?, completed_at = NOW() WHERE id = ?`,
			status, result, id,
		)
	} else {
		res, err = r.db.ExecContext(ctx,
	...
	if err != nil {
		return fmt.Errorf("mysql.PipelineRepo.UpdateJobStatus: %w", err)
	}
...
func (r *PipelineRepo) ListJobs(ctx context.Context, pipelineID string) ([]*repo.PipelineJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.ListJobs: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyJobs(rows)
}
...
func (r *PipelineRepo) CreateStepResult(ctx context.Context, result *repo.StepResult) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.PipelineRepo.CreateStepResult: %w", err)
	}
...
func (r *PipelineRepo) ListStepResults(ctx context.Context, jobID string) ([]*repo.StepResult, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.ListStepResults: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *PipelineRepo) SearchPipelines(ctx context.Context, query string, workspaceName string) ([]*repo.Pipeline, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.SearchPipelines: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyPipelines(rows)
}
...
func (r *PipelineRepo) ListJobsFiltered(ctx context.Context, pipelineID string, filter repo.JobFilter) ([]*repo.PipelineJob, error) {
	...
	rows, err := r.db.QueryContext(ctx, sqlStr, args...)
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.ListJobsFiltered: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyJobs(rows)
}
...
func (r *PipelineRepo) CreateAssignment(ctx context.Context, assignment *repo.PipelineAssignment) (string, error) {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO pipeline_assignments (id, pipeline_id, workspace_id, project_id) VALUES (?, ?, ?, ?)`,
		assignment.ID, assignment.PipelineID, assignment.WorkspaceID, assignment.ProjectID,
	)
	if err != nil {
		return "", fmt.Errorf("mysql.PipelineRepo.CreateAssignment: %w", err)
	}
...
func (r *PipelineRepo) ListAssignments(ctx context.Context, workspaceID string, pipelineID string) ([]*repo.PipelineAssignment, error) {
	...
	rows, err := r.db.QueryContext(ctx, sqlStr, args...)
	if err != nil {
		return nil, fmt.Errorf("mysql.PipelineRepo.ListAssignments: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *PipelineRepo) DeleteAssignment(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, `DELETE FROM pipeline_assignments WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("mysql.PipelineRepo.DeleteAssignment: %w", err)
	}
	affected, _ := res.RowsAffected()
	if affected == 0 {
...
func scanMyPipelines(rows *sql.Rows) ([]*repo.Pipeline, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &description, &p.WorkspaceName, &projectName,
			...
			return nil, fmt.Errorf("mysql.scanMyPipelines: %w", err)
		}
		p.Description = description.String
		p.ProjectName = projectName.String
...
func scanMyJobs(rows *sql.Rows) ([]*repo.PipelineJob, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&j.ID, &j.PipelineID, &j.Status, &j.WorkspaceName,
			...
			return nil, fmt.Errorf("mysql.scanMyJobs: %w", err)
		}
		if startedAt.Valid {
			t := startedAt.Time
			j.StartedAt = &t
