...
// ProjectRepo implements repo.ProjectRepo for MySQL.
...
func (r *ProjectRepo) CreateProjectPlan(ctx context.Context, plan *repo.ProjectPlan) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.ProjectRepo.CreateProjectPlan: %w", err)
	}
	return plan.ID, nil
}
...
func (r *ProjectRepo) GetProjectPlan(ctx context.Context, id string) (*repo.ProjectPlan, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, name, COALESCE(description, ''), workspace_name, status, priority, created_at, updated_at
		 FROM project_plans WHERE id = ?`, id,
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.GetProjectPlan: %w", err)
	}
...
func (r *ProjectRepo) ListProjectPlans(ctx context.Context, workspaceName string) ([]*repo.ProjectPlan, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, COALESCE(description, ''), workspace_name, status, priority, created_at, updated_at
		 FROM project_plans WHERE workspace_name = ? ORDER BY created_at`, workspaceName,
	)
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListProjectPlans: %w", err)
	}
...
func (r *ProjectRepo) DeleteProjectPlan(ctx context.Context, id string) error {
	...
	if _, err := r.db.ExecContext(ctx,
		...
		return fmt.Errorf("mysql.ProjectRepo.DeleteProjectPlan(tasks): %w", err)
	}
	if _, err := r.db.ExecContext(ctx,
		`DELETE FROM milestones WHERE project_id = ?`, id); err != nil {
		return fmt.Errorf("mysql.ProjectRepo.DeleteProjectPlan(milestones): %w", err)
	}
	...
	if _, err := r.db.ExecContext(ctx,
		...
		return fmt.Errorf("mysql.ProjectRepo.DeleteProjectPlan(comments): %w", err)
	}

	result, err := r.db.ExecContext(ctx, `DELETE FROM project_plans WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.DeleteProjectPlan: %w", err)
	}
...
func (r *ProjectRepo) UpdateProjectStatus(ctx context.Context, id string, status string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE project_plans SET status = ?, updated_at = NOW() WHERE id = ?`, status, id,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.UpdateProjectStatus: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("project plan %q not found", id)
...
func (r *ProjectRepo) MoveProjectWorkspace(ctx context.Context, id string, targetWorkspace string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE project_plans SET workspace_name = ?, updated_at = NOW() WHERE id = ?`, targetWorkspace, id,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.MoveProjectWorkspace: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("project plan %q not found", id)
...
func (r *ProjectRepo) CreateMilestone(ctx context.Context, milestone *repo.Milestone) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.ProjectRepo.CreateMilestone: %w", err)
	}
...
func (r *ProjectRepo) GetMilestone(ctx context.Context, id string) (*repo.Milestone, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.GetMilestone: %w", err)
	}
...
func (r *ProjectRepo) ListMilestones(ctx context.Context, projectID string) ([]*repo.Milestone, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, project_id, name, COALESCE(description, ''), status, priority, due_date, order_index, COALESCE(assignee_agent_id, '')
		 FROM milestones WHERE project_id = ? ORDER BY order_index`, projectID,
	)
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListMilestones: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *ProjectRepo) AssignMilestone(ctx context.Context, milestoneID string, agentID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE milestones SET assignee_agent_id = ? WHERE id = ?`, agentID, milestoneID,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.AssignMilestone: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
...
func (r *ProjectRepo) UnassignMilestone(ctx context.Context, milestoneID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE milestones SET assignee_agent_id = '' WHERE id = ?`, milestoneID,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.UnassignMilestone: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("milestone %q not found", milestoneID)
...
func (r *ProjectRepo) CreateTask(ctx context.Context, task *repo.Task) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.ProjectRepo.CreateTask: %w", err)
	}
	return task.ID, nil
}
...
func (r *ProjectRepo) GetTask(ctx context.Context, id string) (*repo.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.GetTask: %w", err)
	}
...
func (r *ProjectRepo) UpdateTaskStatus(ctx context.Context, id string, status string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.UpdateTaskStatus: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
...
func (r *ProjectRepo) ListTasks(ctx context.Context, milestoneID string) ([]*repo.Task, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, milestone_id, name, COALESCE(description, ''), status, priority, order_index, COALESCE(assignee_agent_id, ''), created_at, updated_at
		 FROM tasks WHERE milestone_id = ? ORDER BY order_index`, milestoneID,
	)
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListTasks: %w", err)
	}
	defer func() { _ = rows.Close() }()

	return scanMyTasks(rows)
}
...
func (r *ProjectRepo) ListTasksByWorkspace(ctx context.Context, workspaceName string, status string) ([]*repo.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListTasksByWorkspace: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *ProjectRepo) AssignTask(ctx context.Context, taskID string, agentID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE tasks SET assignee_agent_id = ?, updated_at = NOW() WHERE id = ?`, agentID, taskID,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.AssignTask: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
...
func (r *ProjectRepo) UnassignTask(ctx context.Context, taskID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE tasks SET assignee_agent_id = '', updated_at = NOW() WHERE id = ?`, taskID,
	)
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.UnassignTask: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("task %q not found", taskID)
...
func (r *ProjectRepo) DeleteMilestone(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.DeleteMilestone: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
...
func (r *ProjectRepo) DeleteTask(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.ProjectRepo.DeleteTask: %w", err)
	}
	rows, _ := result.RowsAffected()
	if rows == 0 {
		return fmt.Errorf("task %q not found", id)
...
func (r *ProjectRepo) PurgeOrphans(ctx context.Context) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("mysql.ProjectRepo.PurgeOrphans(tasks): %w", err)
	}
	...
	if err != nil {
		return total, fmt.Errorf("mysql.ProjectRepo.PurgeOrphans(milestones): %w", err)
	}
	n, _ = res.RowsAffected()
	total += n
	...
	if err != nil {
		return total, fmt.Errorf("mysql.ProjectRepo.PurgeOrphans(comments): %w", err)
	}
	n, _ = res.RowsAffected()
	total += n
...
func (r *ProjectRepo) ReconcileCompletionStatus(ctx context.Context) (int, int, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("mysql.ProjectRepo.ReconcileCompletionStatus(milestones): %w", err)
	}
	...
	if err != nil {
		return int(msCount), 0, fmt.Errorf("mysql.ProjectRepo.ReconcileCompletionStatus(projects): %w", err)
	}
...
func (r *ProjectRepo) ListTasksByAgent(ctx context.Context, agentID string, status string, projectID string) ([]*repo.Task, error) {
	...
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListTasksByAgent: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *ProjectRepo) GetNextTask(ctx context.Context, agentID string) (*repo.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.GetNextTask: %w", err)
	}
...
func (r *ProjectRepo) AddComment(ctx context.Context, comment *repo.Comment) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.ProjectRepo.AddComment: %w", err)
	}
...
func (r *ProjectRepo) ListComments(ctx context.Context, entityType, entityID string) ([]*repo.Comment, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.ProjectRepo.ListComments: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
// scanMyTasks scans task rows for MySQL (time.Time scanned directly with parseTime=true).
func scanMyTasks(rows *sql.Rows) ([]*repo.Task, error) {
	var tasks []*repo.Task
	for rows.Next() {
		task := &repo.Task{}
		var description, assigneeID sql.NullString
		...
		if err := rows.Scan(&task.ID, &task.MilestoneID, &task.Name, &description, &task.Status, &task.Priority, &task.OrderIndex, &assigneeID, &createdAt, &updatedAt); err != nil {
			return nil, fmt.Errorf("mysql.scanMyTasks: %w", err)
		}
		task.Description = description.String
		task.AssigneeAgentID = assigneeID.String
		task.CreatedAt = createdAt
