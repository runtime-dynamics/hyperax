...
// CronRepo implements repo.CronRepo for MySQL.
...
func (r *CronRepo) CreateJob(ctx context.Context, job *repo.CronJob) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.CronRepo.CreateJob: %w", err)
	}
	return job.ID, nil
}
...
func (r *CronRepo) GetJob(ctx context.Context, id string) (*repo.CronJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.CronRepo.GetJob: %w", err)
	}
...
func (r *CronRepo) ListJobs(ctx context.Context) ([]*repo.CronJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.CronRepo.ListJobs: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyCronJobs(rows)
}
...
func (r *CronRepo) UpdateJob(ctx context.Context, job *repo.CronJob) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.UpdateJob: %w", err)
	}
...
func (r *CronRepo) DeleteJob(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.DeleteJob: %w", err)
	}
...
func (r *CronRepo) GetDueJobs(ctx context.Context, now time.Time) ([]*repo.CronJob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.CronRepo.GetDueJobs: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanMyCronJobs(rows)
}
...
func (r *CronRepo) UpdateNextRun(ctx context.Context, id string, nextRun time.Time) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE cron_jobs SET next_run_at = ?, updated_at = NOW() WHERE id = ?`,
		nextRun, id,
	)
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.UpdateNextRun: %w", err)
	}
...
func (r *CronRepo) UpdateLastRun(ctx context.Context, id string, lastRun time.Time, status string) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE cron_jobs SET last_run_at = ?, last_status = ?, updated_at = NOW() WHERE id = ?`,
		lastRun, status, id,
	)
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.UpdateLastRun: %w", err)
	}
...
func (r *CronRepo) CreateExecution(ctx context.Context, exec *repo.CronExecution) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.CronRepo.CreateExecution: %w", err)
	}
	return exec.ID, nil
}
...
func (r *CronRepo) GetHistory(ctx context.Context, jobID string, limit int) ([]*repo.CronExecution, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.CronRepo.GetHistory: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *CronRepo) CompleteExecution(ctx context.Context, id string, status string, errMsg string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.CompleteExecution: %w", err)
	}
...
func (r *CronRepo) AddToDLQ(ctx context.Context, entry *repo.CronDLQEntry) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.AddToDLQ: %w", err)
	}
	return nil
}
...
func (r *CronRepo) ListDLQ(ctx context.Context) ([]*repo.CronDLQEntry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.CronRepo.ListDLQ: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *CronRepo) RetryFromDLQ(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.RetryFromDLQ: %w", err)
	}

	// Re-enable the cron job if it still exists.
	if _, err := r.db.ExecContext(ctx,
		`UPDATE cron_jobs SET enabled = 1, updated_at = NOW() WHERE id = ?`,
		cronJobID,
	); err != nil {
		slog.Error("failed to re-enable cron job after DLQ recovery", "job_id", cronJobID, "error", err)
		return fmt.Errorf("mysql.CronRepo.RetryFromDLQ: re-enable cron job %s: %w", cronJobID, err)
	}
...
func (r *CronRepo) DeleteFromDLQ(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("mysql.CronRepo.DeleteFromDLQ: %w", err)
	}
...
// scanMyCronJobs scans rows into CronJob slices.
func scanMyCronJobs(rows *sql.Rows) ([]*repo.CronJob, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, fmt.Errorf("mysql.scanMyCronJobs: %w", err)
		}
