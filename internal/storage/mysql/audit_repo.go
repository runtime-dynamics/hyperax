...
// AuditRepo implements repo.AuditRepo for MySQL.
...
func (r *AuditRepo) CreateAudit(ctx context.Context, audit *repo.Audit) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.AuditRepo.CreateAudit: %w", err)
	}
	return audit.ID, nil
}
...
func (r *AuditRepo) ListAudits(ctx context.Context, workspaceName string) ([]*repo.Audit, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.AuditRepo.ListAudits: %w", err)
	}
...
// GetAuditItem retrieves a single audit item by its ID.
func (r *AuditRepo) GetAuditItem(ctx context.Context, itemID string) (*repo.AuditItem, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.AuditRepo.GetAuditItem: %w", err)
	}
...
func (r *AuditRepo) GetAuditItems(ctx context.Context, auditID string) ([]*repo.AuditItem, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.AuditRepo.GetAuditItems: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *AuditRepo) UpdateAuditItem(ctx context.Context, id string, status string, findings string) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE audit_items SET status = ?, findings = ?, reviewed_at = NOW() WHERE id = ?`,
		status, findings, id,
	)
	if err != nil {
		return fmt.Errorf("mysql.AuditRepo.UpdateAuditItem: %w", err)
	}
...
func (r *AuditRepo) GetAuditProgress(ctx context.Context, auditID string) (*repo.AuditProgress, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.AuditRepo.GetAuditProgress: %w", err)
	}
	defer func() { _ = rows.Close() }()
