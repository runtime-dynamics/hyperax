...
func (r *GitRepo) GetInfo(ctx context.Context, workspaceName string) (*repo.GitInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("mysql.GitRepo.GetInfo: %w", err)
	}
...
func (r *GitRepo) ListSubmodules(ctx context.Context, workspaceName string) ([]*repo.Submodule, error) {
	rootPath, err := r.resolveWorkspacePath(ctx, workspaceName)
	if err != nil {
		return nil, fmt.Errorf("mysql.GitRepo.ListSubmodules: %w", err)
	}
...
func (r *GitRepo) GetRecentChanges(ctx context.Context, workspaceName string, limit int, pathFilter string) ([]*repo.RecentChange, error) {
	rootPath, err := r.resolveWorkspacePath(ctx, workspaceName)
	if err != nil {
		return nil, fmt.Errorf("mysql.GitRepo.GetRecentChanges: %w", err)
	}
	...
	return myParseRecentChanges(out), nil
}

// myParseRecentChanges splits the git log --name-only output into structured
// RecentChange entries.
func myParseRecentChanges(output string) []*repo.RecentChange {
	...
	for _, line := range strings.Split(output, "\n") {
		...
		if myIsCommitHeader(line) {
			if current != nil {
				changes = append(changes, current)
			}
			hash, subject := mySplitCommitHeader(line)
...
// myIsCommitHeader returns true if the line looks like a "hash subject" commit
// header from git log --format="%h %s".
func myIsCommitHeader(line string) bool {
	spaceIdx := strings.IndexByte(line, ' ')
	if spaceIdx < 1 {
		return myIsHexString(line) && len(line) >= 7
	}
	hash := line[:spaceIdx]
	return len(hash) >= 7 && len(hash) <= 12 && myIsHexString(hash)
}

// mySplitCommitHeader splits "hash subject" into its two parts.
func mySplitCommitHeader(line string) (hash, subject string) {
	spaceIdx := strings.IndexByte(line, ' ')
	if spaceIdx < 0 {
		return line, ""
	}
...
// myIsHexString returns true if every byte in s is a hexadecimal digit.
func myIsHexString(s string) bool {
	for i := 0; i < len(s); i++ {
		c := s[i]
...
func (r *GitRepo) resolveWorkspacePath(ctx context.Context, workspaceName string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("mysql.GitRepo.resolveWorkspacePath: resolve workspace %q: %w", workspaceName, err)
	}
...
func (r *GitRepo) runGit(ctx context.Context, dir string, args ...string) (string, error) {
	...
	out, err := cmd.Output()
	if err != nil {
		return "", fmt.Errorf("mysql.GitRepo.runGit: git %s: %w", strings.Join(args, " "), err)
	}
	return string(out), nil
}

// myParseGitmodules reads a .gitmodules INI-style file and extracts submodule entries.
func myParseGitmodules(f *os.File) ([]*repo.Submodule, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "[submodule") {
			...
			name := myExtractSubmoduleName(line)
		...
		key, value, ok := myParseINILine(line)
		if !ok {
			continue
		}
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("mysql.myParseGitmodules: %w", err)
	}

	return submodules, nil
}

// myExtractSubmoduleName pulls the quoted name out of a [submodule "name"] header.
func myExtractSubmoduleName(line string) string {
...
// myParseINILine splits a "key = value" line.
func myParseINILine(line string) (key, value string, ok bool) {
