...
// AuditRepo implements repo.AuditRepo for PostgreSQL.
...
func (r *AuditRepo) CreateAudit(ctx context.Context, audit *repo.Audit) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.AuditRepo.CreateAudit: %w", err)
	}
	return audit.ID, nil
}
...
func (r *AuditRepo) ListAudits(ctx context.Context, workspaceName string) ([]*repo.Audit, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, workspace_name, COALESCE(project_name, ''), status, audit_type,
		 ...
		 FROM audits WHERE workspace_name = $1 ORDER BY created_at DESC`, workspaceName,
	)
	if err != nil {
		return nil, fmt.Errorf("postgres.AuditRepo.ListAudits: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&scopeDesc, &a.CreatedAt, &a.UpdatedAt,
		); err != nil {
			return nil, fmt.Errorf("postgres.AuditRepo.ListAudits: %w", err)
		}
...
func (r *AuditRepo) GetAuditItem(ctx context.Context, itemID string) (*repo.AuditItem, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, audit_id, item_type, COALESCE(file_path, ''), COALESCE(symbol_name, ''),
		 ...
		 FROM audit_items WHERE id = $1`, itemID,
	).Scan(
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.AuditRepo.GetAuditItem: %w", err)
	}
...
func (r *AuditRepo) GetAuditItems(ctx context.Context, auditID string) ([]*repo.AuditItem, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, audit_id, item_type, COALESCE(file_path, ''), COALESCE(symbol_name, ''),
		        status, context_data, findings, reviewed_at
		 FROM audit_items WHERE audit_id = $1 ORDER BY item_type, file_path`, auditID,
	)
	if err != nil {
		return nil, fmt.Errorf("postgres.AuditRepo.GetAuditItems: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *AuditRepo) UpdateAuditItem(ctx context.Context, id string, status string, findings string) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE audit_items SET status = $1, findings = $2, reviewed_at = NOW() WHERE id = $3`,
		status, findings, id,
	)
	if err != nil {
		return fmt.Errorf("postgres.AuditRepo.UpdateAuditItem: %w", err)
	}
...
func (r *AuditRepo) GetAuditProgress(ctx context.Context, auditID string) (*repo.AuditProgress, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT status, COUNT(*) FROM audit_items WHERE audit_id = $1 GROUP BY status`, auditID,
	)
	if err != nil {
		return nil, fmt.Errorf("postgres.AuditRepo.GetAuditProgress: %w", err)
	}
	defer func() { _ = rows.Close() }()
