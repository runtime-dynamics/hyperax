...
// TokenRepo implements repo.MCPTokenRepo for PostgreSQL.
...
func (r *TokenRepo) Create(ctx context.Context, token *types.MCPToken) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.TokenRepo.Create: %w", err)
	}
...
func (r *TokenRepo) ValidateToken(ctx context.Context, plaintext string) (*types.MCPToken, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.TokenRepo.ValidateToken: %w", err)
	}
	defer func() { _ = rows.Close() }()

	for rows.Next() {
		token, err := scanPgToken(rows)
		if err != nil {
			return nil, fmt.Errorf("postgres.TokenRepo.ValidateToken: %w", err)
		}
...
func (r *TokenRepo) Revoke(ctx context.Context, tokenID string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.TokenRepo.Revoke: %w", err)
	}
...
func (r *TokenRepo) ListByAgent(ctx context.Context, personaID string) ([]*types.MCPToken, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.TokenRepo.ListByAgent: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *TokenRepo) DeleteExpired(ctx context.Context) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("postgres.TokenRepo.DeleteExpired: %w", err)
	}
...
func (r *TokenRepo) GetByID(ctx context.Context, tokenID string) (*types.MCPToken, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.TokenRepo.GetByID: %w", err)
	}

	if err := json.Unmarshal([]byte(scopesRaw), &t.Scopes); err != nil {
		return nil, fmt.Errorf("postgres.TokenRepo: unmarshal scopes: %w", err)
	}
	if expiresAt.Valid {
...
// scanPgToken extracts a token from a row scanner (sql.Rows).
func scanPgToken(rows *sql.Rows) (*types.MCPToken, error) {
	var (
		t         types.MCPToken
		scopesRaw string
		expiresAt sql.NullTime
	...
	if err := rows.Scan(
		...
		return nil, fmt.Errorf("postgres.scanPgToken: %w", err)
	}
