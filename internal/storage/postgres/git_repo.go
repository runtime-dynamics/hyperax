...
// WorkspaceRepo. This implementation is backend-agnostic (no database queries).
type GitRepo struct {
...
func (r *GitRepo) GetInfo(ctx context.Context, workspaceName string) (*repo.GitInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.GitRepo.GetInfo: %w", err)
	}
...
func (r *GitRepo) ListSubmodules(ctx context.Context, workspaceName string) ([]*repo.Submodule, error) {
	rootPath, err := r.resolveWorkspacePath(ctx, workspaceName)
	if err != nil {
		return nil, fmt.Errorf("postgres.GitRepo.ListSubmodules: %w", err)
	}
...
func (r *GitRepo) GetRecentChanges(ctx context.Context, workspaceName string, limit int, pathFilter string) ([]*repo.RecentChange, error) {
	rootPath, err := r.resolveWorkspacePath(ctx, workspaceName)
	if err != nil {
		return nil, fmt.Errorf("postgres.GitRepo.GetRecentChanges: %w", err)
	}
	...
	return pgParseRecentChanges(out), nil
}

// pgParseRecentChanges splits the git log --name-only output into structured
// RecentChange entries.
func pgParseRecentChanges(output string) []*repo.RecentChange {
	...
	for _, line := range strings.Split(output, "\n") {
		...
		if pgIsCommitHeader(line) {
			if current != nil {
				changes = append(changes, current)
			}
			hash, subject := pgSplitCommitHeader(line)
...
// pgIsCommitHeader returns true if the line looks like a "hash subject" commit
// header from git log --format="%h %s".
func pgIsCommitHeader(line string) bool {
	spaceIdx := strings.IndexByte(line, ' ')
	if spaceIdx < 1 {
		return pgIsHexString(line) && len(line) >= 7
	}
	hash := line[:spaceIdx]
	return len(hash) >= 7 && len(hash) <= 12 && pgIsHexString(hash)
}

// pgSplitCommitHeader splits "hash subject" into its two parts.
func pgSplitCommitHeader(line string) (hash, subject string) {
	spaceIdx := strings.IndexByte(line, ' ')
	if spaceIdx < 0 {
		return line, ""
	}
...
// pgIsHexString returns true if every byte in s is a hexadecimal digit.
func pgIsHexString(s string) bool {
	for i := 0; i < len(s); i++ {
		c := s[i]
...
func (r *GitRepo) resolveWorkspacePath(ctx context.Context, workspaceName string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.GitRepo.resolveWorkspacePath: resolve workspace %q: %w", workspaceName, err)
	}
...
func (r *GitRepo) runGit(ctx context.Context, dir string, args ...string) (string, error) {
	...
	out, err := cmd.Output()
	if err != nil {
		return "", fmt.Errorf("postgres.GitRepo.runGit: git %s: %w", strings.Join(args, " "), err)
	}
	return string(out), nil
}

// pgParseGitmodules reads a .gitmodules INI-style file and extracts submodule entries.
func pgParseGitmodules(f *os.File) ([]*repo.Submodule, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "[submodule") {
			...
			name := pgExtractSubmoduleName(line)
		...
		key, value, ok := pgParseINILine(line)
		if !ok {
			continue
		}
	...
	if err := scanner.Err(); err != nil {
		return nil, fmt.Errorf("postgres.pgParseGitmodules: %w", err)
	}

	return submodules, nil
}

// pgExtractSubmoduleName pulls the quoted name out of a [submodule "name"] header.
func pgExtractSubmoduleName(line string) string {
...
// pgParseINILine splits a "key = value" line.
func pgParseINILine(line string) (key, value string, ok bool) {
