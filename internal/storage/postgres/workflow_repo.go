...
// WorkflowRepo implements repo.WorkflowRepo for PostgreSQL.
...
func (r *WorkflowRepo) CreateWorkflow(ctx context.Context, wf *repo.Workflow) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.WorkflowRepo.CreateWorkflow: %w", err)
	}
	return wf.ID, nil
}
...
func (r *WorkflowRepo) GetWorkflow(ctx context.Context, id string) (*repo.Workflow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.GetWorkflow: %w", err)
	}
	return wf, nil
}
...
func (r *WorkflowRepo) ListWorkflows(ctx context.Context) ([]*repo.Workflow, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.ListWorkflows: %w", err)
	}
...
func (r *WorkflowRepo) UpdateWorkflow(ctx context.Context, wf *repo.Workflow) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.UpdateWorkflow: %w", err)
	}
...
func (r *WorkflowRepo) DeleteWorkflow(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.DeleteWorkflow: %w", err)
	}
...
func (r *WorkflowRepo) CreateStep(ctx context.Context, step *repo.WorkflowStep) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.WorkflowRepo.CreateStep: %w", err)
	}
	return step.ID, nil
}
...
func (r *WorkflowRepo) GetSteps(ctx context.Context, workflowID string) ([]*repo.WorkflowStep, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.GetSteps: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *WorkflowRepo) UpdateStep(ctx context.Context, step *repo.WorkflowStep) error {
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE workflow_steps SET
		    name = $1, step_type = $2, action = $3, depends_on = $4,
		    condition = $5, requires_approval = $6, position = $7,
		    updated_at = NOW()
		 WHERE id = $8`,
	...
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.UpdateStep: %w", err)
	}
...
func (r *WorkflowRepo) DeleteStep(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, "DELETE FROM workflow_steps WHERE id = $1", id)
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.DeleteStep: %w", err)
	}
...
func (r *WorkflowRepo) CreateRun(ctx context.Context, run *repo.WorkflowRun) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.WorkflowRepo.CreateRun: %w", err)
	}
	return run.ID, nil
}
...
func (r *WorkflowRepo) GetRun(ctx context.Context, id string) (*repo.WorkflowRun, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.GetRun: %w", err)
	}
...
func (r *WorkflowRepo) ListRuns(ctx context.Context, workflowID string) ([]*repo.WorkflowRun, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.ListRuns: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *WorkflowRepo) UpdateRunStatus(ctx context.Context, id string, status string, errMsg string) error {
	...
	switch status {
	case "running":
		q = `UPDATE workflow_runs SET status = $1, started_at = NOW(), updated_at = NOW() WHERE id = $2`
		args = []interface{}{status, id}
	case "completed", "failed", "cancelled":
		q = `UPDATE workflow_runs SET status = $1, error = $2, finished_at = NOW(), updated_at = NOW() WHERE id = $3`
		args = []interface{}{status, errMsg, id}
	default:
		q = `UPDATE workflow_runs SET status = $1, error = $2, updated_at = NOW() WHERE id = $3`
		args = []interface{}{status, errMsg, id}
	}

	res, err := r.db.ExecContext(ctx, q, args...)
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.UpdateRunStatus: %w", err)
	}
...
func (r *WorkflowRepo) CreateRunStep(ctx context.Context, rs *repo.WorkflowRunStep) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.WorkflowRepo.CreateRunStep: %w", err)
	}
	return rs.ID, nil
}
...
func (r *WorkflowRepo) GetRunSteps(ctx context.Context, runID string) ([]*repo.WorkflowRunStep, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.GetRunSteps: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *WorkflowRepo) UpdateRunStepStatus(ctx context.Context, id string, status string, output string, errMsg string) error {
	...
	switch status {
	case "running":
		q = `UPDATE workflow_run_steps SET status = $1, started_at = NOW(), updated_at = NOW() WHERE id = $2`
		args = []interface{}{status, id}
	case "completed", "failed", "skipped":
		q = `UPDATE workflow_run_steps SET status = $1, output = $2, error = $3, finished_at = NOW(), updated_at = NOW() WHERE id = $4`
		args = []interface{}{status, output, errMsg, id}
	default:
		q = `UPDATE workflow_run_steps SET status = $1, output = $2, error = $3, updated_at = NOW() WHERE id = $4`
		args = []interface{}{status, output, errMsg, id}
	}

	res, err := r.db.ExecContext(ctx, q, args...)
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.UpdateRunStepStatus: %w", err)
	}
...
func (r *WorkflowRepo) CreateTrigger(ctx context.Context, trigger *repo.WorkflowTrigger) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("postgres.WorkflowRepo.CreateTrigger: %w", err)
	}
...
func (r *WorkflowRepo) ListTriggers(ctx context.Context, workflowID string) ([]*repo.WorkflowTrigger, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.WorkflowRepo.ListTriggers: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *WorkflowRepo) DeleteTrigger(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, "DELETE FROM workflow_triggers WHERE id = $1", id)
	if err != nil {
		return fmt.Errorf("postgres.WorkflowRepo.DeleteTrigger: %w", err)
	}
