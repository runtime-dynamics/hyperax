...
// MetricsRepo implements repo.MetricsRepo for PostgreSQL.
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO tool_metrics (tool_name, call_count, total_duration_ms, last_used)
		 ...
		 ON CONFLICT(tool_name) DO UPDATE SET
		   ...
		   total_duration_ms = tool_metrics.total_duration_ms + EXCLUDED.total_duration_ms,
		   last_used = EXCLUDED.last_used`,
		toolName, durationMS,
	)
	if err != nil {
		return fmt.Errorf("postgres.MetricsRepo.RecordToolMetric: %w", err)
	}
	return nil
}
...
func (r *MetricsRepo) GetToolMetrics(ctx context.Context) ([]*repo.ToolMetric, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.MetricsRepo.GetToolMetrics: %w", err)
	}
