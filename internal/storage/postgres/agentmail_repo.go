...
// AgentMailRepo implements repo.AgentMailRepo for PostgreSQL.
...
func (r *AgentMailRepo) Enqueue(ctx context.Context, mail *types.AgentMail, direction string) error {
	...
	_, err := r.db.ExecContext(ctx,
		...
		mail.Encrypted, ackMs, mail.SchemaID,
		direction, mail.SentAt.UTC(),
	)
	if err != nil {
		return fmt.Errorf("postgres.AgentMailRepo.Enqueue: %w", err)
	}
	return nil
}
...
func (r *AgentMailRepo) Dequeue(ctx context.Context, direction string, limit int) ([]*types.AgentMail, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.Dequeue: %w", err)
	}
	...
	if len(ids) > 0 {
		placeholders := pgMailPlaceholders(1, len(ids))
...
func (r *AgentMailRepo) Peek(ctx context.Context, direction string, limit int) ([]*types.AgentMail, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.Peek: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *AgentMailRepo) GetByID(ctx context.Context, id string) (*types.AgentMail, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, from_id, to_id, workspace_id, priority, payload, pgp_signature,
		        encrypted, ack_deadline_ms, schema_id, sent_at
		 FROM agentmail_messages WHERE id = $1`, id,
	).Scan(
		&m.ID, &m.From, &m.To, &m.WorkspaceID,
		&m.Priority, &payload, &m.PGPSignature,
		&m.Encrypted, &ackMs, &m.SchemaID, &m.SentAt,
	)
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("mail %q not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.GetByID: %w", err)
	}
...
func (r *AgentMailRepo) Delete(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, "DELETE FROM agentmail_messages WHERE id = $1", id)
	if err != nil {
...
func (r *AgentMailRepo) CountByDirection(ctx context.Context, direction string) (int, error) {
	var count int
	err := r.db.QueryRowContext(ctx,
		"SELECT COUNT(*) FROM agentmail_messages WHERE direction = $1", direction,
	).Scan(&count)
	if err != nil {
		return 0, fmt.Errorf("postgres.AgentMailRepo.CountByDirection: %w", err)
	}
	return count, nil
}
...
func (r *AgentMailRepo) Acknowledge(ctx context.Context, ack *types.MailAck) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.AgentMailRepo.Acknowledge: %w", err)
	}
	return nil
}
...
func (r *AgentMailRepo) GetAck(ctx context.Context, mailID string) (*types.MailAck, error) {
	...
	err := r.db.QueryRowContext(ctx,
		"SELECT mail_id, instance_id, acked_at, status FROM agentmail_acks WHERE mail_id = $1",
		mailID,
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.GetAck: %w", err)
	}
	return ack, nil
}
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT m.id, m.from_id, m.to_id, m.workspace_id, m.priority, m.payload,
		 ...
		 WHERE a.mail_id IS NULL
		   AND m.sent_at + (m.ack_deadline_ms || ' milliseconds')::INTERVAL < NOW()
		 ORDER BY m.sent_at ASC
		 LIMIT $1`,
		limit,
	)
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.ListUnacknowledged: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *AgentMailRepo) QuarantineToDLO(ctx context.Context, entry *types.DeadLetterEntry) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.AgentMailRepo.QuarantineToDLO: %w", err)
	}
	return nil
}
...
func (r *AgentMailRepo) ListDLO(ctx context.Context, limit int) ([]*types.DeadLetterEntry, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.AgentMailRepo.ListDLO: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *AgentMailRepo) RemoveFromDLO(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx, "DELETE FROM agentmail_dead_letters WHERE id = $1", id)
	if err != nil {
...
// scanPgMail scans an AgentMail row from the standard column set.
func scanPgMail(rows *sql.Rows) (*types.AgentMail, error) {
	m := &types.AgentMail{}
	var payload string
	var ackMs int64
	...
	if err := rows.Scan(
		...
		return nil, fmt.Errorf("postgres.scanPgMail: %w", err)
	}
...
// pgMailPlaceholders generates "$start, $start+1, ..., $start+count-1".
func pgMailPlaceholders(start, count int) string {
