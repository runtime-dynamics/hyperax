...
// DelegationRepo implements repo.DelegationRepo for PostgreSQL.
...
func (r *DelegationRepo) Create(ctx context.Context, d *types.Delegation) error {
	...
	if err != nil {
		return fmt.Errorf("postgres.DelegationRepo.Create: %w", err)
	}
...
func (r *DelegationRepo) GetByID(ctx context.Context, id string) (*types.Delegation, error) {
	...
	d, err := scanPgDelegation(row)
	if err == sql.ErrNoRows {
		return nil, repo.ErrNotFound
	}
	if err != nil {
		return nil, fmt.Errorf("postgres.DelegationRepo.GetByID: %w", err)
	}
	return d, nil
}
...
func (r *DelegationRepo) ListByGrantee(ctx context.Context, granteeID string) ([]*types.Delegation, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.DelegationRepo.ListByGrantee: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanPgDelegations(rows)
}
...
func (r *DelegationRepo) ListByGranter(ctx context.Context, granterID string) ([]*types.Delegation, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.DelegationRepo.ListByGranter: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanPgDelegations(rows)
}
...
func (r *DelegationRepo) ListAll(ctx context.Context) ([]*types.Delegation, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.DelegationRepo.ListAll: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanPgDelegations(rows)
}
...
func (r *DelegationRepo) Revoke(ctx context.Context, id string) error {
	res, err := r.db.ExecContext(ctx,
		`UPDATE delegations SET revoked_at = NOW()::TEXT WHERE id = $1 AND revoked_at IS NULL`, id)
	if err != nil {
		return fmt.Errorf("postgres.DelegationRepo.Revoke: %w", err)
	}
...
func (r *DelegationRepo) CleanupExpired(ctx context.Context) (int, error) {
	res, err := r.db.ExecContext(ctx,
		`UPDATE delegations SET revoked_at = NOW()::TEXT
		 WHERE revoked_at IS NULL AND expires_at IS NOT NULL AND expires_at <= NOW()::TEXT`)
	if err != nil {
		return 0, fmt.Errorf("postgres.DelegationRepo.CleanupExpired: %w", err)
	}
...
// scanPgDelegation scans a single delegation row.
func scanPgDelegation(row *sql.Row) (*types.Delegation, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres.scanPgDelegation: %w", err)
	}
	...
	if scopesJSON.Valid && scopesJSON.String != "" {
		if err := json.Unmarshal([]byte(scopesJSON.String), &d.Scopes); err != nil {
			return nil, fmt.Errorf("postgres.scanPgDelegation: unmarshal scopes: %w", err)
		}
	}
	if expiresAt.Valid {
...
// scanPgDelegations scans multiple delegation rows.
func scanPgDelegations(rows *sql.Rows) ([]*types.Delegation, error) {
	...
	for rows.Next() {
		...
		if err != nil {
			return nil, fmt.Errorf("postgres.scanPgDelegations: %w", err)
		}
		...
		if scopesJSON.Valid && scopesJSON.String != "" {
			if err := json.Unmarshal([]byte(scopesJSON.String), &d.Scopes); err != nil {
				return nil, fmt.Errorf("postgres.scanPgDelegations: unmarshal scopes: %w", err)
			}
		}
		if expiresAt.Valid {
			d.ExpiresAt = expiresAt.String
