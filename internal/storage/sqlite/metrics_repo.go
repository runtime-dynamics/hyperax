...
// MetricsRepo implements repo.MetricsRepo for SQLite.
...
func (r *MetricsRepo) RecordToolMetric(ctx context.Context, toolName string, durationMS int64) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.MetricsRepo.RecordToolMetric: %w", err)
	}
	return nil
}
...
func (r *MetricsRepo) GetToolMetrics(ctx context.Context) ([]*repo.ToolMetric, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.MetricsRepo.GetToolMetrics: %w", err)
	}
	...
	for rows.Next() {
		...
		if lastUsed.Valid {
			t, err := time.Parse("2006-01-02 15:04:05", lastUsed.String)
			if err == nil {
				m.LastUsed = &t
			}
		}
