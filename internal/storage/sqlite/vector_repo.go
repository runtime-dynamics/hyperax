...
// VectorRepo implements repo.VectorRepo for SQLite, storing float32 embeddings
// as little-endian BLOB columns. Distance computation is performed in Go since
// the project uses modernc.org/sqlite (pure Go, no CGO) and cannot load the
...
var _ repo.VectorRepo = (*VectorRepo)(nil)
...
func (r *VectorRepo) UpsertSymbolEmbedding(ctx context.Context, symbolID, workspaceID string, embedding []float32, dim int, model string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.VectorRepo.UpsertSymbolEmbedding: %w", err)
	}
	return nil
}
...
func (r *VectorRepo) UpsertDocChunkEmbedding(ctx context.Context, chunkID, workspaceID string, embedding []float32, dim int, model string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.VectorRepo.UpsertDocChunkEmbedding: %w", err)
	}
	return nil
}
...
func (r *VectorRepo) GetSymbolEmbeddings(ctx context.Context, workspaceIDs []string) ([]repo.EmbeddingRecord, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.VectorRepo.GetSymbolEmbeddings: %w", err)
	}
...
func (r *VectorRepo) GetDocChunkEmbeddings(ctx context.Context, workspaceIDs []string) ([]repo.EmbeddingRecord, error) {
	...
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("sqlite.VectorRepo.GetDocChunkEmbeddings: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *VectorRepo) DeleteSymbolEmbedding(ctx context.Context, symbolID string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.VectorRepo.DeleteSymbolEmbedding: %w", err)
	}
	return nil
}
...
func (r *VectorRepo) DeleteDocChunkEmbedding(ctx context.Context, chunkID string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.VectorRepo.DeleteDocChunkEmbedding: %w", err)
	}
	return nil
}
...
func (r *VectorRepo) CountSymbolEmbeddings(ctx context.Context, workspaceIDs []string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("sqlite.VectorRepo.CountSymbolEmbeddings: %w", err)
	}
	return count, nil
}
...
// encodeEmbedding converts a []float32 slice into a little-endian byte slice
...
// decodeEmbedding converts a little-endian byte slice back into a []float32.
// Returns nil if the blob length is not a multiple of 4 or mismatches the
// expected dimension.
...
// scanEmbeddingRecords reads rows from a query that returns (id, workspace_id, embedding, dim).
func scanEmbeddingRecords(rows *sql.Rows) ([]repo.EmbeddingRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&id, &workspaceID, &blob, &dim); err != nil {
			return nil, fmt.Errorf("sqlite.scanEmbeddingRecords: %w", err)
		}

		embedding := decodeEmbedding(blob, dim)
		if embedding == nil {
			// Corrupted or dimension-mismatched embedding — skip silently.
			continue
		}
