...
// ProjectRepo implements repo.ProjectRepo for SQLite.
...
func (r *ProjectRepo) CreateProjectPlan(ctx context.Context, plan *repo.ProjectPlan) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite.ProjectRepo.CreateProjectPlan: %w", err)
	}
	return plan.ID, nil
}
...
func (r *ProjectRepo) GetProjectPlan(ctx context.Context, id string) (*repo.ProjectPlan, error) {
	...
	err := r.db.QueryRowContext(ctx,
		...
		id,
	).Scan(&plan.ID, &plan.Name, &description, &plan.WorkspaceName, &plan.Status, &plan.Priority, &createdAt, &updatedAt)

	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("project plan %q not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.GetProjectPlan: %w", err)
	}
...
// ListProjectPlans returns all project plans for a given workspace, ordered by creation time.
func (r *ProjectRepo) ListProjectPlans(ctx context.Context, workspaceName string) ([]*repo.ProjectPlan, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, name, COALESCE(description, ''), workspace_name, status, priority, created_at, updated_at
		 FROM project_plans WHERE workspace_name = ? ORDER BY created_at`,
		workspaceName,
	)
	if err != nil {
	...
	for rows.Next() {
		...
		var createdAt, updatedAt string
		var description sql.NullString
		if err := rows.Scan(&plan.ID, &plan.Name, &description, &plan.WorkspaceName, &plan.Status, &plan.Priority, &createdAt, &updatedAt); err != nil {
			return nil, fmt.Errorf("sqlite.ProjectRepo.ListProjectPlans: %w", err)
		}
		plan.Description = description.String
		var parseErr error
...
func (r *ProjectRepo) DeleteProjectPlan(ctx context.Context, id string) error {
	...
	if _, err := r.db.ExecContext(ctx,
		`DELETE FROM tasks WHERE milestone_id IN (SELECT id FROM milestones WHERE project_id = ?)`, id); err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteProjectPlan(tasks): %w", err)
	}
	if _, err := r.db.ExecContext(ctx,
		`DELETE FROM milestones WHERE project_id = ?`, id); err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteProjectPlan(milestones): %w", err)
	}
	// Delete orphaned comments referencing any of these entities.
	if _, err := r.db.ExecContext(ctx,
		`DELETE FROM comments WHERE entity_type = 'project' AND entity_id = ?`, id); err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteProjectPlan(comments): %w", err)
	}

	result, err := r.db.ExecContext(ctx, `DELETE FROM project_plans WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteProjectPlan: %w", err)
	}
...
// UpdateProjectStatus changes the status of a project plan (e.g. pending, in-progress, completed, archived).
func (r *ProjectRepo) UpdateProjectStatus(ctx context.Context, id string, status string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE project_plans SET status = ?, updated_at = datetime('now') WHERE id = ?`,
		status, id,
	)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UpdateProjectStatus: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UpdateProjectStatus: %w", err)
...
func (r *ProjectRepo) MoveProjectWorkspace(ctx context.Context, id string, targetWorkspace string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE project_plans SET workspace_name = ?, updated_at = datetime('now') WHERE id = ?`,
		targetWorkspace, id,
	)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.MoveProjectWorkspace: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.MoveProjectWorkspace: %w", err)
...
func (r *ProjectRepo) CreateMilestone(ctx context.Context, milestone *repo.Milestone) (string, error) {
	...
	if milestone.DueDate != nil {
		s := milestone.DueDate.Format("2006-01-02 15:04:05")
		dueDateStr = &s
	}
	...
	_, err := r.db.ExecContext(ctx,
		...
		milestone.Status, milestone.Priority, dueDateStr, milestone.OrderIndex, milestone.AssigneeAgentID,
	)
	if err != nil {
		return "", fmt.Errorf("sqlite.ProjectRepo.CreateMilestone: %w", err)
	}
...
func (r *ProjectRepo) GetMilestone(ctx context.Context, id string) (*repo.Milestone, error) {
	ms := &repo.Milestone{}
	var description, dueDateStr, assigneeID sql.NullString

	err := r.db.QueryRowContext(ctx,
		`SELECT id, project_id, name, COALESCE(description, ''), status, priority, due_date, order_index, COALESCE(assignee_agent_id, '')
		 FROM milestones WHERE id = ?`,
		id,
	).Scan(&ms.ID, &ms.ProjectID, &ms.Name, &description, &ms.Status, &ms.Priority, &dueDateStr, &ms.OrderIndex, &assigneeID)

	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("milestone %q not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.GetMilestone: %w", err)
	}

	ms.Description = description.String
	ms.AssigneeAgentID = assigneeID.String
	if dueDateStr.Valid && dueDateStr.String != "" {
		t, err := time.Parse("2006-01-02 15:04:05", dueDateStr.String)
		if err == nil {
			ms.DueDate = &t
		}
	}
...
func (r *ProjectRepo) ListMilestones(ctx context.Context, projectID string) ([]*repo.Milestone, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, project_id, name, COALESCE(description, ''), status, priority, due_date, order_index, COALESCE(assignee_agent_id, '')
		 FROM milestones WHERE project_id = ? ORDER BY order_index`,
		projectID,
	)
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.ListMilestones: %w", err)
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		ms := &repo.Milestone{}
		var description, dueDateStr, assigneeID sql.NullString
		if err := rows.Scan(&ms.ID, &ms.ProjectID, &ms.Name, &description, &ms.Status, &ms.Priority, &dueDateStr, &ms.OrderIndex, &assigneeID); err != nil {
			return nil, fmt.Errorf("sqlite.ProjectRepo.ListMilestones: %w", err)
		}
		ms.Description = description.String
		ms.AssigneeAgentID = assigneeID.String
...
func (r *ProjectRepo) AssignMilestone(ctx context.Context, milestoneID string, agentID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE milestones SET assignee_agent_id = ? WHERE id = ?`,
		agentID, milestoneID,
	)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.AssignMilestone: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.AssignMilestone: %w", err)
...
func (r *ProjectRepo) UnassignMilestone(ctx context.Context, milestoneID string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UnassignMilestone: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UnassignMilestone: %w", err)
...
func (r *ProjectRepo) CreateTask(ctx context.Context, task *repo.Task) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite.ProjectRepo.CreateTask: %w", err)
	}
	return task.ID, nil
}
...
func (r *ProjectRepo) GetTask(ctx context.Context, id string) (*repo.Task, error) {
	...
	err := r.db.QueryRowContext(ctx,
		`SELECT id, milestone_id, name, COALESCE(description, ''), status, priority, order_index, COALESCE(assignee_agent_id, ''), created_at, updated_at
		 FROM tasks WHERE id = ?`,
		id,
	).Scan(&task.ID, &task.MilestoneID, &task.Name, &description, &task.Status, &task.Priority, &task.OrderIndex, &assigneeID, &createdAt, &updatedAt)

	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("task %q not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.GetTask: %w", err)
	}
...
func (r *ProjectRepo) UpdateTaskStatus(ctx context.Context, id string, status string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UpdateTaskStatus: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UpdateTaskStatus: %w", err)
...
func (r *ProjectRepo) ListTasks(ctx context.Context, milestoneID string) ([]*repo.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.ListTasks: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
// optionally filtered by status. This joins tasks -> milestones -> project_plans.
func (r *ProjectRepo) ListTasksByWorkspace(ctx context.Context, workspaceName string, status string) ([]*repo.Task, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.ListTasksByWorkspace: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *ProjectRepo) AssignTask(ctx context.Context, taskID string, agentID string) error {
	result, err := r.db.ExecContext(ctx,
		`UPDATE tasks SET assignee_agent_id = ?, updated_at = datetime('now') WHERE id = ?`,
		agentID, taskID,
	)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.AssignTask: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.AssignTask: %w", err)
...
func (r *ProjectRepo) UnassignTask(ctx context.Context, taskID string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UnassignTask: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.UnassignTask: %w", err)
...
func (r *ProjectRepo) DeleteMilestone(ctx context.Context, id string) error {
	result, err := r.db.ExecContext(ctx, `DELETE FROM milestones WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteMilestone: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteMilestone: %w", err)
...
func (r *ProjectRepo) DeleteTask(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteTask: %w", err)
	}
	rows, err := result.RowsAffected()
	if err != nil {
		return fmt.Errorf("sqlite.ProjectRepo.DeleteTask: %w", err)
...
func (r *ProjectRepo) PurgeOrphans(ctx context.Context) (int64, error) {
	...
	// Delete orphaned tasks first (their milestones don't exist).
...
// completed and projects where all milestones are completed. Only milestones/
// projects with at least one child are considered. Returns how many of each
// were transitioned to completed.
func (r *ProjectRepo) ReconcileCompletionStatus(ctx context.Context) (int, int, error) {
	// Step 1: auto-complete milestones where every task is completed.
	...
	pjResult, err := r.db.ExecContext(ctx, `
		UPDATE project_plans SET status = 'completed', updated_at = datetime('now')
		WHERE status != 'completed'
...
// ListTasksByAgent returns tasks assigned to a persona, optionally filtered by status
// and/or project ID. Joins tasks -> milestones -> project_plans to resolve project scope.
func (r *ProjectRepo) ListTasksByAgent(ctx context.Context, agentID string, status string, projectID string) ([]*repo.Task, error) {
	...
	rows, err := r.db.QueryContext(ctx, query, args...)
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.ListTasksByAgent: %w", err)
	}
	defer func() { _ = rows.Close() }()
...
func (r *ProjectRepo) GetNextTask(ctx context.Context, agentID string) (*repo.Task, error) {
	...
	if agentID != "" {
		query = `SELECT t.id, t.milestone_id, p.id, t.name, COALESCE(t.description, ''), t.status, t.priority, t.order_index, COALESCE(t.assignee_agent_id, ''), t.created_at, t.updated_at
			...
			INNER JOIN milestones m ON t.milestone_id = m.id
			INNER JOIN project_plans p ON m.project_id = p.id
			WHERE t.assignee_agent_id = ? AND t.status IN ('pending', 'in-progress')
			  AND p.status NOT IN ('completed', 'archived')
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.ProjectRepo.GetNextTask: %w", err)
	}
...
func (r *ProjectRepo) AddComment(ctx context.Context, comment *repo.Comment) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite.ProjectRepo.AddComment: %w", err)
	}
	return comment.ID, nil
}

// ListComments returns all comments for a given entity, ordered by creation time.
func (r *ProjectRepo) ListComments(ctx context.Context, entityType, entityID string) ([]*repo.Comment, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, entity_type, entity_id, content, COALESCE(author, ''), created_at
		 FROM comments WHERE entity_type = ? AND entity_id = ? ORDER BY created_at`,
		entityType, entityID,
	)
	if err != nil {
