...
// InterjectionRepo implements repo.InterjectionRepo for SQLite.
...
func (r *InterjectionRepo) Create(ctx context.Context, ij *types.Interjection) (string, error) {
	...
	if ij.ExpiresAt != nil {
		expiresAt = sql.NullString{String: ij.ExpiresAt.UTC().Format("2006-01-02 15:04:05"), Valid: true}
	}
	...
	_, err := r.db.ExecContext(ctx,
		...
		nullStr(ij.CreatedBy), ij.SourceClearance,
		nullStr(ij.RemediationPersona), nullStr(ij.TrustLevel),
		nullStr(ij.TraceID), expiresAt,
	)
	if err != nil {
		return "", fmt.Errorf("sqlite.InterjectionRepo.Create: %w", err)
	}
...
func (r *InterjectionRepo) GetByID(ctx context.Context, id string) (*types.Interjection, error) {
	...
	ij, err := scanInterjection(row)
	if err == sql.ErrNoRows {
		return nil, fmt.Errorf("interjection %q not found", id)
	}
	if err != nil {
		return nil, fmt.Errorf("sqlite.InterjectionRepo.GetByID: %w", err)
	}
	return ij, nil
}
...
func (r *InterjectionRepo) GetActive(ctx context.Context, scope string) ([]*types.Interjection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.InterjectionRepo.GetActive: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanInterjections(rows)
}
...
func (r *InterjectionRepo) GetAllActive(ctx context.Context) ([]*types.Interjection, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.InterjectionRepo.GetAllActive: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanInterjections(rows)
}
...
func (r *InterjectionRepo) GetHistory(ctx context.Context, scope string, limit int) ([]*types.Interjection, error) {
	...
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, scope, severity, source, reason, status, resolution,
		 ...
		 LIMIT ?`, scope, limit)
	if err != nil {
		return nil, fmt.Errorf("sqlite.InterjectionRepo.GetHistory: %w", err)
	}
	defer func() { _ = rows.Close() }()
	return scanInterjections(rows)
}
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE interjections
		 SET status = 'resolved', resolution = ?, action = ?, resolved_by = ?,
		     resolver_clearance = ?, resolved_at = datetime('now')
		 WHERE id = ? AND status = 'active'`,
		action.Resolution, action.Action, action.ResolvedBy,
		action.ResolverClearance, id,
	)
	if err != nil {
		return fmt.Errorf("sqlite.InterjectionRepo.Resolve: %w", err)
	}
...
func (r *InterjectionRepo) Expire(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.InterjectionRepo.Expire: %w", err)
	}
	return nil
}
...
func (r *InterjectionRepo) GetClearanceLevel(ctx context.Context, personaID string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("sqlite.InterjectionRepo.GetClearanceLevel: %w", err)
	}
	return level, nil
}
...
func (r *InterjectionRepo) CreateBypass(ctx context.Context, bypass *types.SieveBypass) (string, error) {
	...
	_, err := r.db.ExecContext(ctx,
		...
		bypass.ExpiresAt.UTC().Format("2006-01-02 15:04:05"), nullStr(bypass.Reason),
	)
	if err != nil {
		return "", fmt.Errorf("sqlite.InterjectionRepo.CreateBypass: %w", err)
	}
...
func (r *InterjectionRepo) GetActiveBypass(ctx context.Context, scope string) ([]*types.SieveBypass, error) {
	rows, err := r.db.QueryContext(ctx,
		`SELECT id, scope, pattern, granted_by, granted_at, expires_at, reason, revoked
		 FROM sieve_bypass
		 WHERE scope = ? AND revoked = 0 AND expires_at > datetime('now')
...
func (r *InterjectionRepo) RevokeBypass(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.InterjectionRepo.RevokeBypass: %w", err)
	}
	return nil
}
...
func (r *InterjectionRepo) EnqueueDLQ(ctx context.Context, entry *types.DLQEntry) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite.InterjectionRepo.EnqueueDLQ: %w", err)
	}
	return entry.ID, nil
}
...
func (r *InterjectionRepo) ListDLQ(ctx context.Context, interjectionID string, limit int) ([]*types.DLQEntry, error) {
	...
	for rows.Next() {
		e := &types.DLQEntry{}
		var queuedAt string
		var replayedAt, dismissedAt sql.NullString
...
func (r *InterjectionRepo) ReplayDLQ(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE interject_dlq SET status = 'replayed', replayed_at = datetime('now') WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("sqlite.InterjectionRepo.ReplayDLQ: %w", err)
	}
	return nil
}
...
func (r *InterjectionRepo) DismissDLQ(ctx context.Context, id string) error {
	_, err := r.db.ExecContext(ctx,
		`UPDATE interject_dlq SET status = 'dismissed', dismissed_at = datetime('now') WHERE id = ?`, id)
	if err != nil {
		return fmt.Errorf("sqlite.InterjectionRepo.DismissDLQ: %w", err)
	}
	return nil
}
...
func (r *InterjectionRepo) CountDLQ(ctx context.Context, interjectionID string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("sqlite.InterjectionRepo.CountDLQ: %w", err)
	}
	return count, nil
}
...
// nullStr converts a string to sql.NullString (empty → NULL).
...
type interjectionScanner interface {
	Scan(dest ...any) error
}

func scanInterjectionRow(s interjectionScanner) (*types.Interjection, error) {
	...
	var resolvedAt, expiresAt sql.NullString
	var sourceClearance int
	var resolverClearance sql.NullInt64

	if err := s.Scan(
		&ij.ID, &ij.Scope, &ij.Severity, &ij.Source, &ij.Reason, &ij.Status,
		&resolution, &createdBy, &sourceClearance, &resolvedBy, &resolverClearance,
		&remediationPersona, &action, &trustLevel, &traceID,
		&createdAt, &resolvedAt, &expiresAt,
	); err != nil {
		return nil, fmt.Errorf("sqlite.scanInterjectionRow: %w", err)
	}
	...
	ij.SourceClearance = sourceClearance
	if resolvedBy.Valid {
...
func scanInterjection(row *sql.Row) (*types.Interjection, error) {
	return scanInterjectionRow(row)
}

func scanInterjections(rows *sql.Rows) ([]*types.Interjection, error) {
	var results []*types.Interjection
	for rows.Next() {
		ij, err := scanInterjectionRow(rows)
		if err != nil {
			return nil, fmt.Errorf("sqlite.scanInterjections: %w", err)
		}
