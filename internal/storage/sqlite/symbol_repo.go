...
// SymbolRepo implements repo.SymbolRepo for SQLite.
type SymbolRepo struct {
	db *sql.DB
}

// UpsertFileHash inserts or updates a file hash record and returns the file_id.
// If the (workspace_id, file_path) pair already exists, the hash_value and
// updated_at columns are updated in place.
func (r *SymbolRepo) UpsertFileHash(ctx context.Context, workspaceID, filePath, hash string) (int64, error) {
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO file_hashes (workspace_id, file_path, hash_value)
		 ...
		 DO UPDATE SET hash_value = excluded.hash_value, updated_at = datetime('now')`,
		workspaceID, filePath, hash,
	)
	if err != nil {
		return 0, fmt.Errorf("sqlite.SymbolRepo.UpsertFileHash: %w", err)
	}
...
// GetFileHash returns the stored hash for a workspace file, or sql.ErrNoRows
...
func (r *SymbolRepo) GetFileHash(ctx context.Context, workspaceID, filePath string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("sqlite.SymbolRepo.GetFileHash: %w", err)
	}
	return hash, nil
}

// Upsert inserts or replaces a symbol record. The symbol's FileID and
// WorkspaceID must be populated before calling.
func (r *SymbolRepo) Upsert(ctx context.Context, sym *repo.Symbol) error {
	_, err := r.db.ExecContext(ctx,
		`INSERT OR REPLACE INTO symbols (file_id, name, kind, start_line, end_line, signature, workspace_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		sym.FileID, sym.Name, sym.Kind, sym.StartLine, sym.EndLine, sym.Signature, sym.WorkspaceID,
	)
	if err != nil {
		return fmt.Errorf("sqlite.SymbolRepo.Upsert: %w", err)
	}
	return nil
}

// GetFileSymbols returns all symbols for a file identified by workspace and
// path. It joins file_hashes to resolve the file_id from the given path.
func (r *SymbolRepo) GetFileSymbols(ctx context.Context, workspaceID, filePath string) ([]*repo.Symbol, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("sqlite.SymbolRepo.GetFileSymbols: %w", err)
	}
...
func (r *SymbolRepo) DeleteByWorkspacePath(ctx context.Context, workspaceID, filePath string) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.SymbolRepo.DeleteByWorkspacePath: %w", err)
	}
...
// DeleteByFile removes all symbols associated with a given file_id. This is
// used before re-indexing a file to ensure stale symbols are cleaned up.
func (r *SymbolRepo) DeleteByFile(ctx context.Context, fileID int64) error {
	...
	if err != nil {
		return fmt.Errorf("sqlite.SymbolRepo.DeleteByFile: %w", err)
	}
	return nil
}
